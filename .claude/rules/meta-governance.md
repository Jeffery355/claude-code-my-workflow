...
## Economics Paper Extension

When this template is used for **empirical economics papers**, the dual-identity framework extends as follows:

| Artifact | Role | Notes |
|---|---|---|
| `papers/[slug]/do-files/*.do` | Authoritative source | All numerical results derive from Stata; LaTeX manuscript is derived |
| `papers/[slug]/manuscript/*.tex` | Derived output | Manuscripts reference tables/figures generated by Stata |
| `papers/[slug]/tables/*.tex` | Derived output | Table numbers must match Stata output exactly |
| `papers/[slug]/outputs/*.dta` | Audit artifacts | Saved regression results for reproducibility audit |
| `PROJECT_BRIEF.md` (root) | Project configuration | One file to configure a new paper (generic, committed) |

**Key rules for economics papers:**
- `.claude/rules/stata-conventions.md` — do-file organization
- `.claude/rules/econometrics-standards.md` — identification and SE conventions
- `.claude/rules/manuscript-writing.md` — paper structure and writing
- `.claude/rules/cross-artifact-review.md` — Stata ↔ LaTeX consistency

**Starting a new paper:** See `NEW_ECON_PAPER_GUIDE.md` for step-by-step onboarding.
